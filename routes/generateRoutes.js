...
// ─── Multer — accept all files, don't reject on MIME type ────────────────────
const upload = multer({
  dest: 'uploads/',
  limits: { fileSize: 10 * 1024 * 1024 },
  fileFilter: (req, file, cb) => cb(null, true), // Accept everything, validate later
});

// ─── Middleware: check user + daily limit ─────────────────────────────────────
  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({
        success: false,
        message: 'User not registered. Please restart the app and register.',
      });
    }
    ...
    // ── Daily limit reached — friendly upgrade message ──────────────────────
    if (user.usageToday >= user.dailyLimit) {
      return res.status(429).json({
        success: false,
        limitReached: true,
        message: `🚫 Daily Limit Reached!\n\nYou have used ${user.usageToday}/${user.dailyLimit} generations today.\n\n⚡ Upgrade to Premium from Home to enjoy:\n✅ Unlimited generations\n✅ Priority support\n✅ HD thumbnails`,
      });
    }
...
router.post('/generate-thumbnail',
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (err.message.includes('timed out')) userMessage = 'Generation timed out. Please try again with a simpler prompt.';
      if (err.message.includes('429')) userMessage = 'AI service is busy. Please wait 30 seconds and try again.';
      if (err.message.includes('422')) userMessage = 'Image format error. Please select a different image (JPG/PNG).';
